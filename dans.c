...
int Detect_Victim() {
    ...
    if (Home_Base_State == 254 || Home_Base_State == 250 || Home_Base_State == 246) {
        return 1;
    }
    return 0;
...
int Travel(int direction, int Dist, int Test_ADC, int Spd, int Wall) {
    ...
    while (Dist_Travelled < Dist) { // while we haven't gone far enough
        ...
        if (average <= 90) {
            ...
            if (error == Desired_Level + k || error == Desired_Level - k) {
                Drive(Spd, Spd);
            }
            if (error >= 0) {
        ...
        // Detect_Victim();
        if (Get_VirtualWall_State() && !Detect_Victim()) { // check if there is a V_Wall
...
>>>>>>> 8117479f9287dcdc2a69402d11308f7fcadb8e1e
        ...
        maze[X_Pos][Y_Pos].Options -= 1;
        Travel(Direction, SQUARE, 0, SPEED, 0);
...
int Get_BumpSensor_State() {
    ...
    eusartSend(7); // virtual wall detect 

    int Bump_Sensor = (eusartRec());
...
void Bump_Wall() {
    ...
    while (temp2 >= -300) { //reverse as far as we travelled since last turn

                temp2 += UpdateDistance();
                
                printf("%c", ENDOFTEXT);
                printf("\n\nrev:%d Tar:\n", temp2); // print how far we went & the target distance
                Console_Render();
            }
            Robot_Stop();
            return;
